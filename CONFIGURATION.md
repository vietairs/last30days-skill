...
| X / Twitter | one of: explicit `AUTH_TOKEN` + `CT0` (Bird CLI), `XAI_API_KEY`, `SCRAPECREATORS_API_KEY`, or opt-in `FROM_BROWSER` (cookie-jar auth) | X items in results | cookie-jar / Bird = free; xAI / ScrapeCreators = paid |
...
# OR explicit cookie-jar opt-in (no key needed; reads your browser session)
